		...
		for (EnemyPlane enemyPlane : this.gameFrame.getEnemyList()) {
			...
			if (this.getRec().intersects(this.gameFrame.getBoss_1().getRec())) {
				...
				if (this.gameFrame.getBoss_1().live < 10) {
					...
					Burst burst = new Burst(null, this.gameFrame.getBoss_1().x, this.gameFrame.getBoss_1().y, 90, 90,
							this.gameFrame);
					this.gameFrame.getAddList().add(burst);
			...
			if (this.getRec().intersects(this.gameFrame.getBoss_2().getRec())) {
				...
				if (this.gameFrame.getBoss_2().live < 10) {
					...
					Burst burst = new Burst(null, this.gameFrame.getBoss_2().x, this.gameFrame.getBoss_2().y, 90, 90,
							this.gameFrame);
					this.gameFrame.getAddList().add(burst);
			...
			if (this.getRec().intersects(this.gameFrame.getBoss_3().getRec())) {
				this.gameFrame.getBoss_3().live -= 1;
				this.gameFrame.getRemoveList().add(this);
				if (this.gameFrame.getBoss_3().live < 10) {
					this.gameFrame.getRemoveList().add(this.gameFrame.getBoss_3());
					Burst burst = new Burst(null, this.gameFrame.getBoss_3().x, this.gameFrame.getBoss_3().y, 90, 90,
							this.gameFrame);
					this.gameFrame.getAddList().add(burst);
					this.gameFrame.getBoss_3().x = -100;
					this.gameFrame.setState(4);
				}
			}
		}
